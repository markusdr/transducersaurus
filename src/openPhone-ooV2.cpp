...
public: 
	...
	Arpa2TextFST ( const char* arpa_lm, string eps_v="<eps>", bool tropical_v=false ) {
	...
	void make_text_arc( string istate, string ostate, string isym, string osym, double weight, bool split=false ){
	...
	vector<string> split_token( string token ) {
		...
		vector<string> pg;
		//phoneme
		pg.push_back(token.substr( 0, token.find_first_of(":") ));
		//grapheme
		pg.push_back(token.substr( token.find_first_of(":")+1, token.size()-(token.find_first_of(":")+1) ));
		return pg;
	}
...
int main( int argc, const char* argv[] ) {
	...
	a2tfst.generateFST();
	a2tfst.print_syms( a2tfst.ssyms, "ssyms" );
	a2tfst.print_syms( a2tfst.isyms, "isyms" );
	a2tfst.print_syms( a2tfst.osyms, "osyms" );
	return 0;
}
